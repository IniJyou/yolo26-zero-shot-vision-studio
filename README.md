...
一个基于 Ultralytics YOLO26 与 YOLOE-26 的目标检测学习项目，提供固定类别检测、开放词汇零样本检测和 Gradio Web 界面。

项目重点不是重新发明 YOLO，而是完整实践模型调用、统一结果建模、异常处理、模型缓存、自动测试和 Web 应用集成。

## 项目功能

- YOLO26：使用 COCO 预训练类别进行免训练图片检测
...
当前版本已完成图片检测，视频检测与正式对比评测正在开发中。

## 页面工作流程

```mermaid
flowchart LR
    UI[Gradio 图片页面] --> Controller[ImageWebController]
    Controller --> Service[ImageInferenceService]
    Service --> Cache[DetectorCache]
    Cache --> Y26[YOLO26Detector]
    Cache --> YE[YOLOEDetector]
    Y26 --> Result[InferenceResult]
    YE --> Result
    Result --> Outputs[RGB 图片 / 表格 / 摘要 / JSON]
```

这套分层让 Gradio 页面不直接依赖 Ultralytics `Results`，后续的视频模块也可以复用配置、检测器和结果转换能力。
...
每次 Web 请求都会在 `runs/web/<request-id>/` 产生独立结果目录。JSON 结构示例：
...
│   ├── web_controller.py          # Gradio 回调适配和中文异常
